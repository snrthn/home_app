...
// 注：req/res 用 any 是为了不引入 @types/express 依赖（运行时 express 已随 platform-express 存在）。
...
function readRoleMap(req: any): Record<string, string> {
...
function writeRoleMap(res: any, map: Record<string, string>, keepRole?: string) {
...
export function clearRoleTokenCookie(res: any, req: any, role?: string) {
  const map = readRoleMap(req);
