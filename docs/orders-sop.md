...
> 创建：2026-08-13
> 最近改写：2026-08-21 **退款/售后审核台账 Phase 1 落地（Refund 表 + 审核端点 + 管理端台账页，详见 refund-aftersale-design.md）**
> 当前状态：**前后端订单闭环已跑通（三端订单页 + 结算台账 + 师傅收入页已建）；分账规则引擎、阶梯退款、地域闸门 P0+P1 已落地；投诉/工单模块 Phase 1 已落地（见 complaints-tickets-design.md）；退款/售后审核台账已落地（见 refund-aftersale-design.md）；剩余 P2/P3：WS 广播按区域过滤、admin 师傅管理审计列、旧凭证支付语义收敛**
